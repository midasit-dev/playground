...
const TextFieldV2Component = (props: { title: string; propKey: 'spacing' }) => {
	const { title, propKey } = props;
	const [guideBoxProps, setGuideBoxProps] = useRecoilState(SelectedLayerGuideBoxPropsState);
	const [value, setValue] = useState(
		(guideBoxProps && guideBoxProps[propKey] ? guideBoxProps[propKey] : 0)?.toString(),
	);
...
const ParentSizeInheritComponent = (props: {
	guideBoxPropsState: [
		GuideBoxProps | null,
		React.Dispatch<React.SetStateAction<GuideBoxProps | null>>,
	];
}) => {
	const { guideBoxPropsState } = props;
	const [guideBoxProps, setGuideBoxProps] = guideBoxPropsState;
	const [value, setValue] = useState(
		(guideBoxProps && guideBoxProps.width === 'inherit' && guideBoxProps.height === 'inherit') ||
			false,
	);
...
const SwitchComponent = (props: {
	...
	const [guideBoxProps, setGuideBoxProps] = guideBoxPropsState;
	const [value, setValue] = useState(
		guideBoxProps && guideBoxProps[propKey] ? guideBoxProps[propKey] : false,
	);
			...
			<GuideBox width='100%' spacing={1}>
				...
				{showPosition && (
					<GuideBox width='100%'>
						...
						<SwitchComponent
							{...guideBoxPropsState}
							title='horizontal space between'
							propKey='horSpaceBetween'
						/>
						...
						<SwitchComponent
							{...guideBoxPropsState}
							title='vertical space between'
							propKey='verSpaceBetween'
						/>
					</GuideBox>
				)}
			</GuideBox>
				...
				<GuideBox width='100%' spacing={1}>
					...
					{showJson && (
						<CodeBlock
							...
							title='properties'
							radius={4}
							children={JSON.stringify(guideBoxProps, null, 2)}
						/>
					)}
				</GuideBox>
			)}
...
const App = () => {
	...
	return (
		<Panel
			...
			variant='shadow2'
			padding={2}
			border={`1px solid ${selectedLayerId ? Color.secondary.main : '#d1d1d1'}`}
			backgroundColor='#fff'
		>
			<GuideBox width='100%' spacing={2} opacity={opacityBySelectedLayerId}>
				<GuideBox width='100%' row horSpaceBetween verCenter>
					<Typography variant='h1'>Selected Layer</Typography>
					<Button color='negative' onClick={() => setSelectedLayerId(null)}>
						Reset
					</Button>
				</GuideBox>
				<GuideBox width='100%' row horSpaceBetween verCenter>
					...
					<Typography
						variant='body1'
						color={getSelectedLayerIdColor(selectedLayerId)}
