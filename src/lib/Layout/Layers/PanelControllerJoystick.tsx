import { IconButton, Icon, Tooltip, TextFieldV2, GuideBox } from '@midasit-dev/moaui';
...
const App = () => {
	...
	const { handleClickPrevDelete, handleClickDelAllBoxes, handleClickAddBox } = useBoxes({
		initializeInputs,
	});
		...
		>
			<GuideBox width='100%' row horSpaceBetween verCenter>
				<GuideBox>
					<GuideBox row>
						...
						<Tooltip title='add top Ctrl + Up'>
							<IconButton
								transparent
								onClick={() => handleClickAddBox('top', getCurrentControllerInputs())}
								color='negative'
							>
					...
					<GuideBox row>
						<Tooltip title='add left Ctrl + Left'>
							<IconButton
								transparent
								onClick={() => handleClickAddBox('left', getCurrentControllerInputs())}
								color='negative'
							>
						...
						<Tooltip title='add bottom Ctrl + Bottom'>
							<IconButton
								transparent
								onClick={() => handleClickAddBox('bottom', getCurrentControllerInputs())}
								color='negative'
							>
								<Icon iconName='ExpandMore' />
							</IconButton>
						...
						<Tooltip title='add right Ctrl + Right'>
							<IconButton
								transparent
								onClick={() => handleClickAddBox('right', getCurrentControllerInputs())}
								color='negative'
							>
				...
				<GuideBox row>
					<Tooltip title='add Ctrl + Enter'>
						<IconButton
							transparent
							onClick={() => handleClickAddBox('default', getCurrentControllerInputs())}
							color='negative'
						>
							<Icon iconName='Add' />
						</IconButton>
			...
			<GuideBox show width='100%' fill='#e8e8e8' padding={1} border='1px solid #ddd'>
				<TextFieldV2
					...
					type='number'
					value={controllerState.spacing.toString()}
				/>
			</GuideBox>
		</GuideBox>
	);
